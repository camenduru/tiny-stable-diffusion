...
for key in list(sd.keys()):
    if 'diffusion_model' in key:
        sd['model.'+key[:22]+key[28:]] = sd.pop(key)
    elif 'model' not in key:
        sd['model.'+key] = sd[key]

config = OmegaConf.load(f"{config}")

model = instantiate_from_config(config.modelUNet) ## which contains model1 and model2

## quantize model
n_bits_w, n_bits_a = 2, 8
from tiny_optimizedSD.quant_scripts.brecq_quant_model import QuantModel_int2inference
from tiny_optimizedSD.quant_scripts.brecq_quant_layer import QuantModule_int2inference
from tiny_optimizedSD.quant_scripts.brecq_adaptive_rounding import SimpleDequantizer
...
model = QuantModel_int2inference(model=model, weight_quant_params=wq_params, act_quant_params=aq_params)
...
for name, module in model.named_modules():
    if isinstance(module, QuantModule_int2inference) and module.ignore_reconstruction is False:
        module.weight_quantizer = SimpleDequantizer(uaq=module.weight_quantizer, weight=module.weight)
